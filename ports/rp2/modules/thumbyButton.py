...
if IS_THUMBY_COLOR or IS_THUMBY_COLOR_LINUX or IS_THUMBY_COLOR_WEB:
    import engine_io
    # CRITICAL perf path. The original ssd1306-driven thumby calls
    # `pressed()` MANY times per frame — both directly from game logic
    # and 6 times per pacing-loop iteration in `thumbyGraphics.update`.
    # The naive Color shim used to call `engine.tick()` here, which
    # ran the FULL engine pipeline (LCD DMA setup + framebuffer clear
    # + node walks + ...) every 8 ms. That ate 6-10 ms per game frame
    # of pure overhead, dropping CosmicSurvivor from a 60 fps target
    # to ~30 fps actual game logic — slow motion despite a "healthy"
    # engine fps reading.
    #
    # We only need to refresh the button state machine to make
    # `is_pressed` reflect the latest GPIO read. `engine_io.update_buttons()`
    # does exactly that: ~10 µs of bit ops, no DMA, no node walks. The
    # full engine pipeline still runs once per game frame inside
    # `display.update()`'s `show()` path, so the LCD updates normally.
else:
