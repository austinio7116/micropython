...
class AudioClass:
    def __init__(self, pwm):
        ...
        self.pwm = pwm
        self.enabled = 1
        # Original Thumby ran the buzzer at fixed 50% duty (0xFFFF//2).
        # On Color we scale this by the engine's global volume so the
        # ThumbyOne system volume slider controls legacy game audio.
    ...
    # Volume-scaled duty cycle. ThumbyOne's global volume is exposed
    # via engine_audio.get_volume() (0.0 = mute, 1.0 = max). On the
    # original Thumby this hook is a no-op since engine_audio doesn't
    # exist there — the import-guarded fast path returns the raw
    # dutyCycle.
    if IS_THUMBY_COLOR or IS_THUMBY_COLOR_LINUX:
        ...
        def _scaled_duty(self):
            ...
            if v < 0.0: v = 0.0
            if v > 1.0: v = 1.0
            return int(self.dutyCycle * v * v * v)
    else:
        @micropython.native
        def _scaled_duty(self):
    ...
    def set(self, freq):
        if(self.enabled):
            ...
            self.pwm.freq(freq)
            self.pwm.duty_u16(self._scaled_duty())
