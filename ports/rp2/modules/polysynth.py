...
# Upstream polysynth.py exposes this list at module level. PSdemo's
# oscilloscope feature reads it (`for i in polysynth.unusedpins:`) to
# compute byte offsets into RP2040 GPIO registers — it ends up reading
# meaningless addresses on RP2350 anyway, but the iteration must not
# fail. We keep the same value as upstream so any code that compares
# against it still works.
unusedpins = [7, 8, 9, 10, 11, 21, 22]
...
# Reference to the AudioChannel object engine_audio.play() returns,
# captured per logical voice. Held so we can adjust .gain when the
# active voice count changes — see _apply_per_voice_gain below.
_audio_chan = [None] * _VOICE_COUNT


def _apply_per_voice_gain():
    """Match upstream polysynth's PIO-mixer behaviour: with N voices
    active, the multiplexed output averages to (sum_of_voices / N), so
    the summed amplitude stays in [-1, +1] regardless of N. Our
    engine_audio mixer just sums voices then clamps, so we have to do
    the divide-by-N ourselves by setting each voice's gain to 1/N. With
    N=1 a voice plays at full amplitude; N=7 each plays at ~14 %, sum
    fits cleanly without clipping. Without this scaling, multiple
    voices summing past ±1.0 produce harsh clipping distortion that
    the ear interprets as added low-frequency content (the symptom
    that sounds like "an octave low" in chord-heavy passages)."""
    if not _AUDIO_OK:
        return
    n = _corecount if _corecount > 0 else 1
    g = 1.0 / float(n)
    for ch in _audio_chan:
        if ch is None:
            continue
        try:
            ch.gain = g
...
def _set_voice(logical, hz):
    ...
    try:
        ...
        if not _tone_active[logical]:
            ch = _engine_audio.play(_tone_res[logical], logical, True)
            _audio_chan[logical] = ch
            _tone_active[logical] = True
            # Apply current per-voice gain immediately so the new voice
            # joins at the same level as its siblings.
            n = _corecount if _corecount > 0 else 1
            try:
                ch.gain = 1.0 / float(n)
            except Exception:
    ...
    if _AUDIO_OK:
        for i in range(_VOICE_COUNT):
            ...
            try:
                ...
                pass
    _apply_per_voice_gain()


def enabled(value=None):
    """Original: get/set active mixer channel count. We track it as
    the divisor for per-voice gain (matching upstream's mixer
    multiplexing — fewer enabled channels means each plays louder)."""
    global _corecount
...
def _audiotick(_dummy=None):
    """Per-tick sequencer step. Wrapped in a top-level try so any
    unexpected exception in the event loop shuts the synth down cleanly
    rather than killing the timer with voices left ringing — that was
    the "stuck on a chord, forced power-off" symptom users hit on the
    end of long songs."""
    try:
        _audiotick_inner()
    except Exception:
        # Best-effort: silence everything and stop the timer so the
        # game's `while polysynth.playing:` loop exits.
        try:
            global playing, _stream
            playing = False
            _stream = None
...
def _audiotick_inner():
    global playing, _eventstart, _stream
    stop_now = True
    ...
    if stop_now:
        ...
        if _timer is not None:
            try:
                ...
                pass
        # Critical: actually silence the engine voices. Without this
        # the song ends but the engine keeps mixing whatever the last
        # active voices were forever, and the game's main loop exits
        # while audio still plays. Was: only the bookkeeping fields
        # got cleared.
        for i in range(_VOICE_COUNT):
            notes[i] = None
            _instruments[i] = None
...
def play(song, ins=None, autoenable=True, loop=False):
    global _stream, _eventstart, playing, _instruments
