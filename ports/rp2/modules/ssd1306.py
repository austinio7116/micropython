# MicroPython SSD1306 OLED driver, I2C and SPI interfaces — modified
# for Thumby. On Thumby Color this module exposes only the
# `SSD1306_DUMMY` shape that thumbyGraphics expects; rendering is
# handled by `thumby_render` directly into the engine framebuffer (no
# Sprite2DNode, no scene-graph round-trip).
...
class SSD1306():
    ...
    if IS_THUMBY_COLOR or IS_THUMBY_COLOR_LINUX:
        # Bridge to the bare-metal renderer. thumbyGraphics's update()
        # ultimately calls self.display.show(); we forward the buffer
        # to thumby_render which handles the 1bpp→RGB565 expand,
        # nearest-neighbour scaling, LB/RB scale picker, and the
        # engine.tick() that DMAs the framebuffer to the LCD.
        def show(self):
